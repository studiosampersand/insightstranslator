...
const I18N = {
  nl: {
    navRewrite:'Herschrijven',navCoach:'Ask Liv',navProfiles:'Testprofielen',localDemo:'Lokale demo',
    eyebrow:'Jouw boodschap. Professioneel afgestemd.',title:'Van ruwe tekst naar een bruikbaar antwoord.',subtitle:'Liv behoudt de inhoud, verzacht onnodige frictie en past structuur, detail en toon aan de ontvanger aan.',
    ...
    inputTitle:'Jouw tekst',inputSub:'Schrijf zoals je het denkt. Liv maakt er een professionele versie van.',clear:'Wissen',roughReply:'Mijn ruwe antwoord',receivedMessage:'Bericht dat ik kreeg',sourcePlaceholder:'Bijvoorbeeld: waarom is het project nog niet af? Dat duurt nu al zo lang. Ik wil graag dat we dit tegen vrijdag afronden.',nothingSent:'Niets wordt automatisch verstuurd',addScreenshot:'Screenshot toevoegen',screenshotSub:'Alleen lokale preview in deze alpha',remove:'Verwijderen',rewriteButton:'Herschrijf met Liv',
    outputTitle:'Professionele versie',outputSub:'Bewerkbaar, controleerbaar en klaar om te kopiëren.',copy:'Kopiëren',emptyTitle:'Liv wacht op je tekst.',emptyText:'De herwerking verschijnt hier naast je originele boodschap.',alternate:'Andere versie',copyReply:'Kopieer antwoord',changedTitle:'Wat Liv verbeterde',adviceTitle:'Verbeter vóór verzending',meetingTitle:'Dit bespreek je mogelijk beter rechtstreeks.',copyMeeting:'Kopieer meetingvoorstel',openTeams:'Open Teams',
    ...
    coachEyebrow:'Interne wegwijzer',coachTitle:'Vraag Liv waar je best terechtkunt.',coachText:'Liv koppelt je vraag aan de juiste dienst, eigenaar en volgende actie. Deze demo bewaart geen gespreksgeschiedenis.',coachIt:'Laptop, VPN, account of infrastructuur werkt niet',coachHr:'HR of bedrijfswagen',coachCrm:'CRM of Salesforce',coachDms:'DMS of documentenbeheer',coachCapacity:'Project heeft extra tijd of mensen nodig',coachConflict:'Moeilijke samenwerking of gevoelige vraag',coachQuestion:'Wat wil je vragen?',coachQuestionSub:'Je hoeft het niet netjes te formuleren.',coachPlaceholder:'Bijvoorbeeld: mijn laptop krijgt geen VPN-verbinding en ik moet dringend thuis kunnen werken.',askLiv:'Vraag het aan Liv',
  ...
  en: {
    navRewrite:'Rewrite',navCoach:'Ask Liv',navProfiles:'Test profiles',localDemo:'Local demo',
    eyebrow:'Your message. Professionally adapted.',title:'From rough wording to a usable reply.',subtitle:'Liv preserves the content, reduces unnecessary friction and adapts structure, detail and tone to the recipient.',
    ...
    inputTitle:'Your text',inputSub:'Write it as you think it. Liv turns it into a professional version.',clear:'Clear',roughReply:'My rough reply',receivedMessage:'Message I received',sourcePlaceholder:'For example: why is the project still not finished? This is taking far too long. I want us to finish it by Friday.',nothingSent:'Nothing is sent automatically',addScreenshot:'Add screenshot',screenshotSub:'Local preview only in this alpha',remove:'Remove',rewriteButton:'Rewrite with Liv',
    outputTitle:'Professional version',outputSub:'Editable, reviewable and ready to copy.',copy:'Copy',emptyTitle:'Liv is waiting for your text.',emptyText:'The rewrite will appear here next to your original message.',alternate:'Another version',copyReply:'Copy reply',changedTitle:'What Liv improved',adviceTitle:'Improve before sending',meetingTitle:'This may be better discussed directly.',copyMeeting:'Copy meeting suggestion',openTeams:'Open Teams',
...
function createAdvice(source,output,lang,recipient,analysis){
  ...
  for(const key of rank.slice(0,2)){
    ...
    if(key==='cool_blue'&&!analysis.hasUrl)items.push({css:'advice-blue',text:nl?'Voeg indien beschikbaar de relevante documentatie, cijfers of project-URL toe. Liv verzint nooit zelf een link.':'Add relevant documentation, figures or the project URL if available. Liv never invents a link.'});
...
function runCoach(){const text=$('#coachText').value.trim();if(!text){toast(t('coachNeed'));return;}const route=ROUTES.find(r=>r.keywords.test(text))||{id:'general',service:'Interne dienstverantwoordelijke',owner:'Je manager of de relevante diensteigenaar',description:'Verduidelijk het gewenste resultaat, de urgentie en welke dienst het proces beheert.',actions:['teams','mail','copy-request']};const intro=state.lang==='nl'?'Liv stelt deze route voor':'Liv suggests this route';$('#coachResult').innerHTML=`<h3>${escapeHtml(intro)}</h3><div class="route-card"><strong>${escapeHtml(route.service)}</strong><small>${escapeHtml(route.owner)}</small></div><p>${escapeHtml(route.description)}</p>${coachActionsHtml(route,text)}`;$('#coachResult').classList.remove('hidden');$$('[data-copy-route]').forEach(btn=>btn.addEventListener('click',()=>copyText(routeDraft(route,text))));}
